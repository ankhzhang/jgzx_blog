...
>>>>>>> 342873a08c9acedef4be5127b5f4f025ca56a31d
...
<<<<<<< HEAD
    ...
    # 列表显示
    list_display = ('title', 'author', 'status_colored', 'created_at', 'reviewed_by', 'reviewed_at')
    list_filter = ('status', 'is_published', 'created_at', 'reviewed_at')
    
    # 搜索功能（全局搜索）
    search_fields = ('title', 'content', 'author__username', 'author__first_name', 'reject_reason')
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('title', 'content', 'author', 'is_published', 'view_count')
        }),
        ('审核信息', {
    ...
    def publish_projects(self, request, queryset):
        ...
        self.message_user(request, f'已成功发布 {updated} 个项目', messages.SUCCESS)
    
    @admin.action(description='下架选中项目')
    def unpublish_projects(self, request, queryset):
        """批量下架项目"""
        updated = queryset.update(is_published=False)
        self.message_user(request, f'已成功下架 {updated} 个项目', messages.SUCCESS)
    ...
    def pending_view(self, request):
        """待审核项目专属视图"""
        pending_projects = Project.objects.filter(status='pending').select_related('author')
